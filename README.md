...
*It researches the galaxy while you sleep, refuses to make things up, and was built — and stress-tested — by a council of AI agents.*
...
- ⚖️ **Built by an AI council.** Every non-trivial change was blind-generated by independent agents, adversarially verified, and only committed on a unanimous pass. The git log *is* the audit trail.
...
## ⚖️ Built by an AI council

This repo's real party trick: **it wasn't vibe-coded.** Every non-trivial change went through **Council-v2** — a verification-architecture protocol where correctness comes from *independence* and *grounding*, not from asking one model nicely:
...
- **A final council** added a self-guarding regression tripwire so a future loop can't silently re-introduce the bug class.

An arbiter declining to launder a plausible-but-wrong consensus — *that's* the protocol working. The full decision trail lives in the council ledger.
...
| F6 strand guard + self-guarding tripwire | ✅ built — council-hardened |
...
<sub>Built with 🛠️ <a href="https://claude.com/claude-code">Claude Code</a> · verified by an AI council · grounded in nothing but the truth</sub>
